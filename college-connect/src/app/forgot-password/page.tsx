...
import { ArrowRight, Loader2, Mail, Lock, ShieldCheck, CheckCircle2 } from "lucide-react";
...
export default function ForgotPasswordPage() {
  ...
  const requestLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg("If that email exists, we've sent a secure OTP. Please check your inbox.");
            ...
            <div className="px-8 pb-10">
              <form onSubmit={requestLink} className="space-y-6">
                ...
                <div className="space-y-2 relative">
                  <Label htmlFor="email" className="text-sm font-bold text-slate-700 dark:text-slate-300 ml-1">Account Email</Label>
                  <div className={`relative flex items-center rounded-2xl border transition-colors duration-300 ${focusedField === 'email' ? 'border-blue-500 ring-4 ring-blue-500/10 bg-white dark:bg-[#0f172a]' : 'border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-[#030712]'}`}>
                ...
                <AnimatePresence>
                  ...
                  {successMsg && (
                    <motion.div
                      ...
                      className="overflow-hidden"
                    >
                      <div className="text-sm font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 p-3.5 rounded-xl border border-emerald-200 dark:border-emerald-500/20 flex flex-col gap-3">
                        ...
                        {previewUrl && (
                          <div className="mt-2 p-3 bg-white dark:bg-slate-900 rounded-lg border border-emerald-200 dark:border-emerald-500/20">
                            <p className="text-xs text-slate-500 dark:text-slate-400 mb-2 font-bold uppercase tracking-wider">Development Preview Inbox:</p>
                            <a
                              href={previewUrl}
                              target="_blank"
                            ...
                            >
                              <span>Click here to view the email link</span>
